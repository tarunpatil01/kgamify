...
import { fetchAdminTickets, postAdminTicketMessage, updateAdminTicket } from '../api';
...
export default function AdminSupportTickets({ isDarkMode }) {
  ...
  const loadTickets = useCallback(async () => {
    ...
    try {
      ...
      setTickets(list);
      if (list.length && !selectedTicketId) setSelectedTicketId(String(list[0]._id));
    } catch (e) {
      setError(e.message || 'Failed to load tickets');
    } finally {
      setLoading(false);
    }
  }, [status, query, selectedTicketId]);
  ...
  useEffect(() => {
    ...
    socket.on('ticket:message', ({ ticketId, message }) => {
      ...
      if (incomingId !== String(selectedTicketId) && message.role === 'user') {
    ...
    socket.on('ticket:typing', ({ ticketId, by, isTyping }) => {
      ...
      if (by !== 'company') return;
      setIsCompanyTyping(Boolean(isTyping));
    });
  ...
  const sendReply = async () => {
    ...
    setReply('');
    try {
      const result = await postAdminTicketMessage(selectedTicket._id, { text });
          ...
          <div className="md:col-span-1 space-y-3">
            {tickets.map((ticket) => (
              ...
              >
                ...
                <div className="text-xs opacity-70 mt-1">{ticket.companyName || 'N/A'} · {ticket.companyEmail}</div>
              ...
              <>
                <div className="flex flex-wrap justify-between items-center gap-2 mb-3">
                  ...
                  <select
                    ...
                    onChange={(e) => onStatusChange(selectedTicket._id, e.target.value)}
                ...
                <div className={`h-72 overflow-y-auto rounded border p-3 mb-3 ${isDarkMode ? 'border-gray-700 bg-gray-900' : 'border-gray-200 bg-gray-50'}`}>
                  {(selectedTicket.messages || []).length === 0 && (
                    <div className="text-xs opacity-70">No replies yet on this ticket.</div>
                  )}
                  {(selectedTicket.messages || []).map((m, idx) => {
                    ...
                    const bubbleClass = fromSystem
                      ...
                      : fromAdmin
                        ...
                        : 'bg-orange-100 text-orange-900 mr-auto';
                  ...
                  {isCompanyTyping && (
                    <div className="text-xs text-orange-600 mt-2">Company is typing...</div>
                  )}
                </div>
