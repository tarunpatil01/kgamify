...
export default function Applications({ isDarkMode }) {
  ...
  if (loading) {
    return (
      <div className={`p-4 ${isDarkMode ? 'text-gray-100 bg-gray-900' : 'text-gray-800 bg-gray-100'}`}>Loading applications...</div>
    );
  }

  if (error) {
    return (
      <div className={`p-4 ${isDarkMode ? 'text-red-300 bg-gray-900' : 'text-red-600 bg-gray-100'}`}>{error}</div>
    );
  }

  return (
  <div className={`min-h-screen p-2 sm:p-4 md:p-6 ${isDarkMode ? 'bg-gray-900 text-gray-100' : 'bg-gray-100 text-gray-900'}`}>
          ...
          {filtered.map((app) => (
            ...
            >
              <div className="flex items-start justify-between">
                <div>
                  <div className="font-semibold text-kgamify-500 text-sm sm:text-base">{app.jobTitle}</div>
      <div className="text-sm mt-1 text-gray-800 dark:text-gray-300">
                    Applicant: <span className="font-medium">{app.applicantName}</span>
                  </div>
                  {app.resume && (
                    <div className="mt-2">
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
        className="text-sm underline text-gray-800 dark:text-gray-200 hover:text-kgamify-500"
                      >
                        View Resume
                      </a>
                    </div>
        ...
        <div className="flex items-center text-xs sm:text-sm text-gray-700 dark:text-gray-300 ml-4">
