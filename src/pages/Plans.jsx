...
export default function Plans({ isDarkMode = false }) {
  ...
  return (
    <div className={`min-h-screen py-0 px-0 flex flex-col items-center ${isDarkMode ? 'bg-gray-900 text-white' : 'bg-gray-50 text-gray-900'}`}>
      {/* Header Section */}
      <div className="w-full bg-gradient-to-r from-[#ff8200] to-[#ffb347] py-12 mb-10 flex flex-col items-center">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 items-stretch">
          {cards.map((p, idx) => {
            ...
            return (
              <div
                key={p.key}
                className={`relative rounded-3xl border ${p.color} bg-white dark:bg-gray-800 p-8 flex flex-col shadow-xl hover:shadow-2xl transition-all duration-200 hover:-translate-y-1
              ...
              >
                <div className="flex items-center gap-3 mb-2">
                  {p.icon}
                  <h3 className={`text-2xl font-bold ${p.accent}`}>{desc.title}</h3>
                </div>
                <div className="mb-4 flex flex-col gap-1">
                  <span className="text-4xl font-extrabold text-[#ff8200]">{desc.price}</span>
                  {desc.altPrice && <span className="text-base text-gray-500 dark:text-gray-300">{desc.altPrice}</span>}
                  {desc.jobPrice && <span className="text-xs text-gray-400 dark:text-gray-400">{desc.jobPrice}</span>}
                </div>
                ...
                {isAddon && (
                  <div className="absolute top-4 right-4 px-3 py-1 bg-green-100 text-green-700 rounded-full text-xs font-bold shadow">Add-On</div>
                )}
              </div>
            );
          })}
